...
def home(request):
    ...
    logined_user_id = User.objects.get(username=request.user.username).id
    for post in posts:
...
def my_posts(request):
    posts = Posts.objects.filter(userId__username=request.user.username).order_by('-post_date')
    main_posts = []
    for post in posts:
        main_posts.append({'post': post, 'user': {'username': request.user.username,
                                                'id': User.objects.get(username=request.user.username).id,
                                                'pic': User.objects.get(
                                                    username=request.user.username).profile_pic}})
...
def liked_posts(request):
    ...
    user = User.objects.get(username=request.user.username)  # დალოგინებული იუზერის  გაფილტრული ობიექტი
...
def search_name(request):
    ...
    if request.method == "POST":
        ...
        if search_filter == 'users':
            ...
            logged_user_id = User.objects.get(username=request.user.username).id
...
def follow_user(request, id):
    ...
    logged_user = User.objects.get(username=request.user.username).id
...
def profile_info_privacy(request, id):
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect(reverse('soc:main_profile', args=(request.user.username,)))
    if request.method == "GET":
        return HttpResponseRedirect(reverse('soc:main_profile', args=(request.user.username,)))
